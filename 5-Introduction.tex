...
Le \acs{MBS} constitue la base de la construction des processus de diffusion employés dans de nombreux domaines tels que la finance, la physique ou la biologie. Ces processus sont modélisés par des \acf{EDS}, qui consistent à enrichir le mouvement brownien en y ajoutant un terme de dérive et un terme de diffusion. Il en résulte une classe de processus dits \textit{d'Itô}.
...
Il est parfois intéressant d'enrichir les processus d'Itô en leur ajoutant une composante de sauts, gouvernée par un processus de Poisson (processus de comptage classique modélisant une file d'attente).
...
\begin{figure}[htb]
    ...
    \caption{Simulation de 10 trajectoire d'un processus de sauts pur $J(t)$ de paramètre $\lambda=5$ avec sauts exponentiels de paramètre $\nu=1.5$}\label{fig:TrajPoi}
\end{figure}
...
Comme évoqué précédemment, cette étude s'intéresse à deux types de problématiques appliquées au processus \acl{CIR}. Il convient donc d'en donner une définition formelle.
...
\end{equation}
Autrement dit, $\tau(x)$ correspond au premier instant où le processus atteint l'une des bornes de l'intervalle $(0, c)$, soit $0$ ou $c$. \\
...
Ce cadre permet de modéliser une situation dans laquelle un agent cherche à influencer dynamiquement l'évolution du processus, à l'aide d'un contrôle $u(x)$, tout en minimisant un coût total cumulé jusqu'à un temps d'arrêt.

\paragraph{Définition: Problème de commande optimale}\label{definition_optimal_control}\mbox{}\\
On considère un processus \acs{CIR} contrôlé $\{X_u(t),\, t \geq 0\}$ de la forme:
\begin{equation}\label{controlled_process}
    dX_u(t) = a[b - X_u(t)]dt + b[X_u(t)]u[X_u(t)]dt + \sigma \sqrt{X_u(t)} dW(t)
\end{equation}
où $u(\cdot)$ est la variable de contrôle.\\

L'objectif est de minimiser une fonction coût de la forme:
\begin{equation}\label{cost_function}
    J(x):=\int_0^{\tau(x)} \left(\frac{1}{2}q[X_u(t)]u^2[X_u(t)] + r[X_u(t)] \right) dt
\end{equation}
où:
\begin{itemize}
    \item $\tau(x)$ désigne le temps de premier passage associé au processus contrôlé $X_u(t)$;
    \item $r(x)\neq0$ est le coût d'état (sans contrôle) associé au niveau $x$;
    \item $b(x)\neq0$ est le coût du contrôle appliqué;
    \item $q(x)>0$ est un poids pénalisant l'intensité du contrôle $u[X_u(t)]$.
\end{itemize}

Ce problème vise à déterminer une stratégie optimale $u^*[X_u(t)]$ minimisant le coût $J(x)$ tout en tenant compte des dynamiques spécifiques du processus \acs{CIR} et des contraintes imposées par les frontières $0$ et $c$.

La fonction valeur associée au problème est alors définie par:
...
\begin{itemize}
    ...
    \item Résoudre un problème de commande optimale associé au processus, en déterminant la fonction valeur \( F(x) = \underset{u}{inf}\mathds{E}[J(x)] \), ainsi que la politique de contrôle optimale.
\end{itemize}
